...
const Header = () => {
  ...
  return (
    <div className="absolute w-screen px-8 py-2 bg-gradient-to-b from-black z-10 flex flex-col md:flex-row md:justify-between">
      <img className="w-44 mx-auto md:mx-0" src={logo} alt="loading" />
      {user && (
        <div className="flex justify-between p-3">
       {showGptSearch&&   <select className="p-2 bg-gray-800 text-white m-2" onClick={handleLanguageChange}>
            {SUPPORTED_LANGUAGES.map((lang)=><option key={lang.identifier} value={lang.identifier}>{lang.name}</option>)}
          </select>}

          <button className="py-2 px-4 m-2 rounded-lg my-2 mx-2 bg-purple-600 text-white" 
